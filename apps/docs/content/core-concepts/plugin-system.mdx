...
### 2. Use Plugins Consistently

If you use plugins, use them everywhere for consistency:
...
const apiWithPath = initContract(contract).use(pathPlugin).build();
const apiWithValidate = initContract(contract).use(validatePlugin).build();
```

### 3. Type-Only Imports for Type Helpers
...
type User = InferResponseBody<typeof api.getUser, 200>;
```

### 4. Validate at Boundaries

Use validation plugins at system boundaries (API responses, user input):
...
- Review [Type Inference](/core-concepts/type-inference) for type extraction
